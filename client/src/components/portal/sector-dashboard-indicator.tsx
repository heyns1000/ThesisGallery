...
export function SectorDashboardIndicator({ sector, className = "" }: SectorDashboardIndicatorProps) {
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`} data-testid="sector-dashboard-indicator">
      <motion.div
        className={`w-2 h-2 rounded-full ${color} animate-pulse`}
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
        transition={{ delay: 0.1 }}
        data-testid={`indicator-pulse-${status}`}
      />
      <Badge 
        variant={status === 'enterprise' ? 'default' : status === 'active' ? 'secondary' : 'outline'} 
        className="text-xs"
      ...
      <span className="text-xs text-gray-500" data-testid="indicator-description">{description}</span>
    </div>
  )
}
