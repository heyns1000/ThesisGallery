...
export function SectorRelationshipMapping() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-96" data-testid="loading-relationships">
  ...
  return (
    <div className="space-y-6" data-testid="sector-relationship-mapping">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Network className="w-8 h-8 text-blue-500" />
          <div>
            <h1 className="text-3xl font-bold dark:text-white" data-testid="title-relationship-mapping">Interactive Sector Relationship Mapping</h1>
            <p className="text-gray-600 dark:text-gray-400">Visualize connections and synergies across the ecosystem</p>
          </div>
        </div>
        <Badge variant="outline" className="text-sm" data-testid="stats-badge">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="border rounded px-3 py-1 text-sm dark:bg-gray-800 dark:text-white dark:border-gray-700"
      ...
      <Tabs value={viewMode} onValueChange={(v) => setViewMode(v as any)} className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="network" data-testid="tab-network">Network View</TabsTrigger>
          <TabsTrigger value="matrix" data-testid="tab-matrix">Matrix View</TabsTrigger>
        ...
        <TabsContent value="network" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                {selectedSectorData ? (
                  <div className="space-y-4" data-testid="sector-details">
                    <div className="text-center">
                      <div className="text-4xl mb-2" data-testid="selected-sector-emoji">{selectedSectorData.emoji}</div>
                      <h3 className="font-semibold text-lg dark:text-white" data-testid="selected-sector-name">{selectedSectorData.name}</h3>
                      <p className="text-sm text-gray-600 dark:text-gray-400" data-testid="selected-sector-brands">{selectedSectorData.brandCount} brands</p>
                    </div>

                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium dark:text-white">Connections:</span>
                        <Badge data-testid="connection-count">{selectedSectorConnections.length}</Badge>
                      </div>
                      
                      <div className="space-y-2">
                        <span className="text-sm font-medium dark:text-white">Connected Sectors:</span>
                        {selectedSectorConnections.slice(0, 5).map((conn, i) => {
                          ...
                          return (
                            <div key={i} className="flex items-center gap-2 text-sm" data-testid={`connected-sector-${i}`}>
                              <span>{connectedSector?.emoji}</span>
                              <span className="flex-1 dark:text-gray-300">{connectedSector?.name}</span>
            ...
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr>
                      <th className="text-left p-2 dark:text-white">Sector</th>
                          ...
                          return (
                            <td key={colSector.id} className="text-center p-2">
                              {relationship ? (
                                ...
                                <div className="w-4 h-4 rounded mx-auto bg-gray-100 dark:bg-gray-800" />
                              )}
                            </td>
                          );
                        })}
            ...
            <CardContent>
              <div className="space-y-4">
                {['Enterprise', 'Infrastructure', 'Professional', 'Standard'].map(tier => {
                  ...
                  return (
                    <div key={tier} className="border dark:border-gray-700 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-3">
                        <Badge variant={tier === 'Enterprise' ? 'default' : 'secondary'} data-testid={`tier-badge-${tier}`}>
                          {tier} Tier
                        </Badge>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {tierSectors.map(sector => (
                          <div
                            key={sector.id}
                            className={cn(
                              "p-3 border dark:border-gray-700 rounded-lg text-center cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800",
                              selectedSector === sector.id && "bg-blue-50 dark:bg-blue-900 border-blue-300"
                            )}
                            onClick={() => setSelectedSector(sector.id)}
                            data-testid={`hierarchy-sector-${sector.id}`}
                          >
                            <div className="text-2xl mb-1">{sector.emoji}</div>
                            <div className="text-xs font-medium dark:text-white">{sector.name.split(' ')[0]}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-blue-600 dark:text-blue-400" data-testid="stat-total-connections">
              {relationships.length}
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400">Active relationships</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-green-600 dark:text-green-400" data-testid="stat-avg-connections">
              {Math.round(relationships.length / (sectors.length || 1))}
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400">Per sector</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-purple-600 dark:text-purple-400" data-testid="stat-network-density">
              {Math.round((relationships.length / (sectors.length * (sectors.length - 1) / 2)) * 100)}%
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400">Connectivity</p>
          </CardContent>
        </Card>
      </div>
    </div>
